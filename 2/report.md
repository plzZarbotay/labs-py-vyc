...
8. **Выводы**: Научились работать с ssh. Научились подключаться к удаленным машинам, использовать базовые команды для достижения своих целей. Научились архивировать файлы и прочее, что актуально, встречается повсеметсно в работе. Кроме того еще раз вспомнили базовые команды, например, tail, head, grep, tar. Лабораторная работа , крутая:)
